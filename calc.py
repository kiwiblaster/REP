...
#******************************************************************************
#<data import and preperation for regression and calc>
...
#******************************************************************************
#<define event dates resp. event windows>

event_dates_switzerland = [
 '2010-12-17',
 '2011-03-11',
 '2011-10-20',
 '2011-11-07',
...
#******************************************************************************
#<function for regression, AR etc., Hypothesis Tests>

def regression_and_calc(master_file,event_date,index_ric,constituent_data,days_before,days_after,index_data,fama_french,car_event1):
   ...
   #define date range
    ...
    TGRANK = pd.DataFrame(columns=['Event_Date','Event_Window','SU','Z','TGRANK'])
    SIGN_TEST = pd.DataFrame(columns=['Event_Date','Event_Window','Z_SIGN'])
    GENERALIZED_SIGN_TEST = pd.DataFrame(columns=['Event_Date','Event_Window','W','P_HAT','Z_GS_SIGN'])
    TEST_STATISTICS = pd.DataFrame(columns=['Event_Date','Event_Window','T_GRANK','Z_GSIGN','Z_SIGN'])
    ...
    with pd.ExcelWriter(index_ric + "_" + event_date + "_" + car_event1 + ".xlsx") as writer:  
...
def call_function(INDEX,event_dates,event_windows,constituent_data,fama_french,index_data):
    
    # Initialisierung des DataFrames außerhalb der Schleifen
    #result_df = pd.DataFrame(columns=['RIC','Event_Date','Event_Window','CAR','VAR_AR_Event','VAR_CAR_Event','STD_CAR_Event','SCAR']) 
    result_df_TGRANK = pd.DataFrame(columns=['Event_Date','Event_Window','SU','Z','TGRANK'])
    result_df_SIGN_TEST = pd.DataFrame(columns=['Event_Date','Event_Window','Z_SIGN'])
    result_df_GENERALIZED_SIGN_TEST = pd.DataFrame(columns=['Event_Date','Event_Window','W','P_HAT','Z_GS_SIGN'])
    result_df_TEST_STATISTICS = pd.DataFrame(columns=['Event_Date','Event_Window','T_GRANK','Z_GSIGN','Z_SIGN'])
...
TGRANK_SSHI,SIGN_TEST_SSHI,GENERALIZED_SIGN_TEST_SSHI,TEST_STATISTICS_SSHI = call_function(INDEX='SSHI',event_dates=event_dates_switzerland,event_windows=event_windows,constituent_data=SSHI,fama_french=fama_french_europe,index_data=SSHI_index)
TGRANK_SSHI.to_excel('TGRANK_SSHI.xlsx',index=False)
SIGN_TEST_SSHI.to_excel('SIGN_TEST_SSHI.xlsx',index=False)
GENERALIZED_SIGN_TEST_SSHI.to_excel('GENERALIZED_SIGN_TEST_SSHI.xlsx',index=False)
TEST_STATISTICS_SSHI.to_excel('TEST_STATISTICS_SSHI.xlsx',index=False)

TGRANK_STOXX,SIGN_TEST_STOXX,GENERALIZED_SIGN_TEST_STOXX,TEST_STATISTICS_STOXX = call_function(INDEX='STOXX',event_dates=event_dates_eu,event_windows=event_windows,constituent_data=STOXX,fama_french=fama_french_europe,index_data=STOXX_index)
TGRANK_STOXX.to_excel('TGRANK_STOXX.xlsx',index=False)
SIGN_TEST_STOXX.to_excel('SIGN_TEST_STOXX.xlsx',index=False)
GENERALIZED_SIGN_TEST_STOXX.to_excel('GENERALIZED_SIGN_TEST_STOXX.xlsx',index=False)
TEST_STATISTICS_STOXX.to_excel('TEST_STATISTICS_STOXX.xlsx',index=False)

TGRANK_SPX,SIGN_TEST_SPX,GENERALIZED_SIGN_TEST_SPX,TEST_STATISTICS_SPX = call_function(INDEX='SPX',event_dates=event_dates_us,event_windows=event_windows,constituent_data=SPX,fama_french=fama_french_us,index_data=SPX_index)
TGRANK_SPX.to_excel('TGRANK_SPX.xlsx',index=False)
SIGN_TEST_SPX.to_excel('SIGN_TEST_SPX.xlsx',index=False)
GENERALIZED_SIGN_TEST_SPX.to_excel('GENERALIZED_SIGN_TEST_SPX.xlsx',index=False)
TEST_STATISTICS_SPX.to_excel('TEST_STATISTICS_SPX.xlsx',index=False)
